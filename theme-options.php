...
?>
	...
	//get google fonts
	$fontsArray = get_option($themename . "_google_fonts");
	...
	if($fontsArray===FALSE || (time()-$fontsArray->last_update>2*7*24*60*60)) {
		...
		update_option($themename . "_google_fonts", $fontsArray);
	}	
	?>
				...
				<ul class="form_field_list">
					...
					<li>
						...
						<select id="header_top_sidebar" name="header_top_sidebar">
							...
							{
								?>
								<option value="<?php echo !empty($theme_options["id"]) ? esc_attr($theme_sidebar["id"]) : ""; ?>"<?php echo (isset($theme_options["header_top_sidebar"]) && $theme_options["header_top_sidebar"]==$theme_sidebar["id"] ? " selected='selected'" : ""); ?>><?php echo $theme_sidebar["title"]; ?></option>
								<?php
							}
							?>
						</select>
					...
					<li id="header_top_right_sidebar_container"<?php echo (isset($theme_options["header_layout_type"]) && (int)$theme_options["header_layout_type"]!=2 ? ' style="display: none;"' : ''); ?>>
						...
						<select id="header_top_right_sidebar" name="header_top_right_sidebar">
							...
							{
								?>
								<option value="<?php echo esc_attr($theme_sidebar["id"]); ?>"<?php echo (isset($theme_options["header_top_right_sidebar"]) && $theme_options["header_top_right_sidebar"]==$theme_sidebar["id"] ? " selected='selected'" : ""); ?>><?php echo $theme_sidebar["title"]; ?></option>
								<?php
							}
							?>
						</select>
					...
					<li>
						...
						<div>
							<select id="header_font" name="header_font">
								<option<?php echo (isset($theme_options["header_font"]) && $theme_options["header_font"]=="" ? " selected='selected'" : ""); ?>  value=""><?php _e("Default", 'medicenter'); ?></option>
								<?php
					...
					<li>
						...
						<div>
							<select id="subheader_font" name="subheader_font">
								<option<?php echo (isset($theme_options["subheader_font"]) && $theme_options["subheader_font"]=="" ? " selected='selected'" : ""); ?>  value=""><?php _e("Default", 'medicenter'); ?></option>
								<?php
								$fontsCount = count($fontsArray->items);
								for($i=0; $i<$fontsCount; $i++)
								{
